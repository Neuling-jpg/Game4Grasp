...
from validation.utils.hand_model import create_hand_model
...
class CMapDataset(Dataset):
    def __init__(
        ...
        self.use_extend_data = use_extend_data
        ...
        if full_dataset:
            self.object_names = dataset_split['train'] + dataset_split['validate']
        else:
        ...
        cmap_metadata = torch.load(dataset_path)['metadata']
        self.cmap_metadata = [m for m in cmap_metadata if m[1] in self.object_names and m[2] in self.robot_names]
        if self.use_extend_data:
            extend_dataset_path = os.path.join(ROOT_DIR, f'data/CMapDataset_filtered/graspopt_extend_cmap_dataset.pt')
            extend_metadata = torch.load(extend_dataset_path)['metadata']
            self.extend_metadata = [m for m in extend_metadata if m[1] in self.object_names and m[2] in self.robot_names]
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            for idx in range(self.batch_size):
                ...
                # if self.use_extend_data=True, there are 50% chance that we use extend data
                if self.use_extend_data and random.random() < 0.5:
                    metadata_robot = [(m[0], m[1]) for m in self.extend_metadata if m[2] == robot_name]
                    if len(metadata_robot)==0:
            ...
            object_pc_batch_graspopt = torch.zeros([self.batch_size, 1024, 3], dtype=torch.float32)
            ...
            for batch_idx in range(self.batch_size):
                ...
                if self.object_pc_type == 'partial':
                    ...
                    # object_path = os.path.join(ROOT_DIR, f'data/PointCloud/object/{name[0]}/{name[1]}.pt')
                    # object_pc = torch.load(object_path)[:, :3]
                    indices_all = torch.randperm(65536)
                    indices = indices_all[:self.num_points]
                    object_pc = self.object_pcs[object_name][indices]
                    indices_graspopt = indices_all[:1024]
                    object_pc_graspopt = self.object_pcs[object_name][indices_graspopt]

                initial_q_batch[batch_idx] = initial_q
                robot_pc_batch[batch_idx] = robot_pc
                object_pc_batch[batch_idx] = object_pc
                object_pc_batch_graspopt[batch_idx] = object_pc_graspopt
            ...
            return {
                ...
                'object_pc': object_pc_batch,
                'object_pc_graspopt': object_pc_batch_graspopt,
            }
...
def create_dataloader(cfg, is_train, 
    ...
    dataset = CMapDataset(
        ...
        batch_size=cfg.ntrial_per_case,
        ...
        use_extend_data = use_extend_data
    )
...
if __name__=="__main__":
    d = CMapDataset(batch_size=1)
    loader = DataLoader(dataset=d, batch_size=1, shuffle=False)
